...
def preprocess_multi_row_evi(conversations, tokenizer, model_args, data_args, is_multimodal):
    conv = get_conv(model_args.conv_type)
    roles = {'human': conv.roles[0], 'gpt': conv.roles[1]}
    query = []
    for i, message in enumerate(conversations): 
    ...
    for i, rou in enumerate(rounds):
        ...
        labels[cur_len:cur_len + ins_len] = IGNORE_INDEX    
        loss_labels[cur_len:cur_len + ins_len] = IGNORE_INDEX
        cur_len += rou_len
    ...
    return dict(input_ids_part1=input_ids_part1, labels_part1=labels_part1, input_ids_part2=input_ids_part2, labels_part2=labels_part2, loss_labels=loss_labels, query=query)
...
def preprocess(conversations, tokenizer, model_args, data_args, is_multimodal):
    if model_args.conv_type == 'plain':
        ...
        if model_args.use_matching:
            ...
            return preprocess_multi_row_evi(conversations, tokenizer, model_args, data_args, is_multimodal)
    else:
    ...
    def __getitem__(self, idx):
        ...
        while True:
            try:
                ...
                print(f'path: {anno["task"]}, {anno["video"]}; question: {anno["conversations"][0]["value"]}; src: {anno["src"] if anno.get("src") else "none"}; tgt: {anno["tgt"] if anno.get("tgt") else "none"},  answer: {anno["conversations"][1]["value"]}')
