...
from d2vlm.model import ETCHAT_MODELS
from d2vlm.train.d2vlm_trainer import ETChatTrainer
from d2vlm.utils.io import load_image, load_video
from d2vlm.utils.tokenization import tokenize
from d2vlm.utils.transforms import get_transform
import debugpy
...
class ModelArguments:
    ...
    use_evidence: Optional[bool] = field(default=False)
    full_question: Optional[bool] = field(default=False)
    ...
    consistency_loss: Optional[bool] = field(default=False)
    arch_module: Optional[str] = field(default='etchat_arch')
...
def preprocess_multi_row_evi(conversations, processed_response, tokenizer, model_args, data_args, is_multimodal):
    conv = get_conv(model_args.conv_type)
    roles = {'human': conv.roles[0], 'gpt': conv.roles[1]}
    query = []
    for i, message in enumerate(conversations): 
    ...
    loss_labels_part_1 = loss_labels[:-1].clone()   
    ignore_mask = loss_labels_part_1!= 32011
    loss_labels_part_1 = loss_labels_part_1.masked_fill(ignore_mask, IGNORE_INDEX)
    loss_labels = torch.cat([loss_labels_part_1, processed_response['label'], processed_response['label'][-1:]], dim=0)
    labels = torch.cat([labels[:-1], processed_response['label'], processed_response['label'][-1:]], dim=0)  
    assert processed_response['label'][-1:].item()==-100
    input_ids = torch.cat([input_ids[:-1], processed_response['full_token_ids'], input_ids[-1:]], dim=0)
    relative_offset = rou_len -1 
    

    # image_token_index = prompt.index(DEFAULT_IMAGE_TOKEN)
    image_token_index = torch.where(input_ids==IMAGE_TOKEN_INDEX)[0]
    input_ids_part1 = input_ids[:image_token_index+1]   
    input_ids_part2 = input_ids[image_token_index+1:]

    labels_part1 = labels[:image_token_index+1]
    labels_part2 = labels[image_token_index+1:]
    loss_labels = loss_labels[image_token_index+1:]
    
    return dict(input_ids_part1=input_ids_part1, labels_part1=labels_part1, input_ids_part2=input_ids_part2, labels_part2=labels_part2, loss_labels=loss_labels, query=query, relative_offset = relative_offset)


def preprocess(conversations, processed_response, tokenizer, model_args, data_args, is_multimodal):
    if model_args.conv_type == 'plain':
        ...
        if model_args.use_matching:
            return preprocess_multi_row(conversations, tokenizer, model_args, data_args, is_multimodal)
        elif model_args.use_evidence:
            return preprocess_multi_row_evi(conversations, processed_response, tokenizer, model_args, data_args, is_multimodal)
    else:
...
def get_target_modules(model, lora_type, language_model):
    ...
    if lora_type == 'linear':
        excludes = ['vision_tower', 'mm_projector', 'qformer', 'lm_head', 'frm_head', 'vid_head', 'evi_head']
        target_modules = set()
...
class MultimodalDataset(Dataset):
    ...
    def __init__(self, tokenizer, model_args, data_args, training_args):
        ...
        raw_annos = torch.load(data_args.anno_path)
    ...
    def __getitem__(self, idx):
        ...
        while True:
            try:
                ...
                # print(f'path: {anno["task"]}, {anno["video"]}; answer: {anno["conversations"][1]["value"]}')
                value = anno["conversations"][0]["value"]
                last_line = value.split("\n")[-1]
                # print(f'path: {anno["task"]}, {anno["video"]}; question = {last_line}')
                ...
                data = preprocess(anno['conversations'], anno['processed_response'], self.tokenizer, self.model_args, self.data_args, mm_type)
        ...
        ####################################
        data['relative_offset'] = data['relative_offset'] + data['image'].shape[0] - 1
        data['pos_id'] = anno['processed_response']['pos_id']
        data['special_mask_list'] = anno['processed_response']['special_mask_list']
        data['conflict_events'] = anno['processed_response']['conflict_events']
        # data['valid_evi_index'] = anno['processed_response']['valid_evi_index']
        data['tgt_re'] = anno.get('tgt_reject')
...
class MultimodalDataCollator(object):
    ...
    def __init__(self, tokenizer, model_args):
        ...
        self.evi_flag = model_args.use_evidence

    def __call__(self, batch):  
        if self.evi_flag:
            input_ids_part1 = [d['input_ids_part1'] for d in batch]
            input_ids_part1 = nn.utils.rnn.pad_sequence(input_ids_part1, batch_first=True, padding_value=self.tokenizer.pad_token_id)
            input_ids_part1 = input_ids_part1[:, :self.tokenizer.model_max_length] 

            input_ids_part2 = [d['input_ids_part2'] for d in batch]

            labels_part1 = [d['labels_part1'] for d in batch]
            labels_part1 = nn.utils.rnn.pad_sequence(labels_part1, batch_first=True, padding_value=IGNORE_INDEX)
            labels_part1 = labels_part1[:, :self.tokenizer.model_max_length]

            labels_part2 = [d['labels_part2'] for d in batch]
            loss_labels = [d['loss_labels'] for d in batch]
            relative_offset = [d['relative_offset'] for d in batch]

            data = dict(input_ids_part1=input_ids_part1, labels_part1=labels_part1, 
                        input_ids_part2=input_ids_part2, labels_part2=labels_part2, 
                        attention_mask1=(input_ids_part1 != self.tokenizer.pad_token_id), 
                        attention_mask2 = [[True for _ in sublist] for sublist in labels_part2],
                        loss_labels = loss_labels, relative_offset = relative_offset
                        )
        else:
        ...
        for key in ('query', 'tag', 'src', 'tgt', 'tgt_re', 'pos_id', 'special_mask_list', 'conflict_events'):
...
def train():
    ...
    config = config_cls.from_pretrained(model_args.model_name_or_path, torch_dtype=dtype, arch_module=model_args.arch_module)
    ...
    if config.use_matching:
        ...
        new_tokens = [DEFAULT_EVIDENCE_TOKEN, DEFAULT_EVIDENCE_END_TOKEN]
        num_tokens = tokenizer.add_special_tokens({
            'additional_special_tokens': new_tokens
        })

        model.config.evidence_token_id = tokenizer.convert_tokens_to_ids(DEFAULT_EVIDENCE_TOKEN)
        model.config.evidence_end_token_id = tokenizer.convert_tokens_to_ids(DEFAULT_EVIDENCE_END_TOKEN)
    ...
    for n, p in model.named_parameters():
        ...
        if config.use_evidence and any(k in n for k in ('embed_tokens', 'lm_head', 'frm_head', 'evi_head')):    
            p.requires_grad = True  
