...
from d2vlm.utils.inference import KeywordsStoppingCriteria
...
def apply_template(data):
    query = f"Carefully watch the video and pay attention to the cause and sequence of events, the detail and movement of objects and the action and pose of persons. Based on your observations, select the best option that accurately addresses the question.\nQuestion: {data['question']}\nOptions:\n"
    answer = data['answer']
...
if __name__ == '__main__':
    ...
    for meta in meta_data:
        anno = nncore.load(nncore.join(args.anno_path, meta[1]))
        ...
        for i in nncore.ProgressBar(range(len(anno))):
            ...
            video_path = nncore.join(args.data_path, meta[2], sample['video'])
