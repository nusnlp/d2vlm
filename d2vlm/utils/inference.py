...
class RepetitionPenaltyLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, penalty, match_token_id, evidence_token_id = None):
        assert isinstance(penalty, (int, float)) and penalty > 0
        self.penalty = penalty
        self.match_token_id = match_token_id
        self.evidence_token_id = evidence_token_id
    ...
    def __call__(self, input_ids, scores):
        ...
        #     (input_ids[0] != -200) * 
        #     (input_ids[0] != self.match_token_id) * 
        #     (input_ids[0] != self.evidence_token_id)
        # ].unsqueeze(0)
