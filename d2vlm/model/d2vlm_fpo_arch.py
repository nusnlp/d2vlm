    ...
    def _update_causal_mask(self, *args, **kwargs):
        ...
        if self.config.bi_attention:
            for idx, seps in enumerate(self.img_token_pos):
                ...
                if not self.test and self.stage_2:
                    for special_mask in self.special_mask_list[idx]:
                        for sub_mask in special_mask[1]:
                            mask[idx, :, special_mask[0]+self.relative_offset[idx], sub_mask[0]+self.relative_offset[idx]:sub_mask[1]+self.relative_offset[idx]+1] = -65504
        return mask
...
class D2VLMMetaForCausalLM:
    ...
    def prepare_multimodal_data2(self, input_ids_part2, inputs_embeds1, processed_image, img_cnt, attention_mask2, labels_part2, attention_mask1, labels_part1, original_lengths1, loss_labels, src, tgt, tgt_re, img_embed_ori):
        
        _src = []
        for s, t, tr in zip(src, tgt, tgt_re):  
            _s = []
            _s += s if s is not None else []
            _s += t if t is not None else []
            _s += tr if tr is not None else []
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids_part2):
            ...
            # len_tgt, len_src = int(len(tgt[batch_idx])/2), int(len(src[batch_idx])/2)
            len_tgt = int(len(tgt[batch_idx])/2) if tgt[batch_idx] else 0
            len_tgt_re = int(len(tgt_re[batch_idx])/2) if tgt[batch_idx] else 0
            ...
            if inds.shape[0] > 0:
                ...
                if _src[batch_idx]:  
                    ...
                    for index in range(len_tgt_re):
                        ...
                        # mean_value_ori = cur_original_image[current_s[0]:current_s[1]+1].mean(dim=0)
                        s = s[2:]
                        non_img_tokens[inds[index+2*len_tgt]] = non_img_tokens[inds[index+2*len_tgt]] + mean_value
                        non_img_tokens[inds[index+2*len_tgt+len_tgt_re]] = non_img_tokens[inds[index+2*len_tgt+len_tgt_re]] + mean_value
                    ...
                    for index in range(len_src):
                        ...
                        mean_value_ori = cur_original_image[current_s[0]:current_s[1]+1].mean(dim=0)
                        s = s[2:]
                        non_img_tokens[inds[index]] =  non_img_tokens[inds[index]] + mean_value_ori
        ...
        loss_labels_pad = []
        pos_id_pad = []
        for i, (emb1, emb2, emb3) in enumerate(zip(labels_part1_unpad, labels_part2, loss_labels)):
            combined = torch.cat([emb1, emb2], dim=0)
            combined_loss = torch.cat([emb1, emb3], dim=0)
            cur_pre_pos_id = torch.arange(0, self.model.relative_offset[i], device=emb1.device)
            cur_pos_id = torch.tensor(self.pos_id[i], device= emb1.device)
            cur_pos_id = cur_pos_id + self.model.relative_offset[i]
            end_pos_id = (cur_pos_id[-1] + 1).unsqueeze(0)
            cur_pos_id = torch.cat([cur_pre_pos_id, cur_pos_id, end_pos_id], dim=0) 
            ...
            if combined.size(0) < max_length:
                ...
                cur_pad_pos_id = torch.arange(cur_pos_id[-1]+1, cur_pos_id[-1]+1+pad_length, device=inputs_embeds1.device)    
                cur_pos_id = torch.cat([cur_pos_id, cur_pad_pos_id], dim=0)

            labels_part2_pad.append(combined)
            loss_labels_pad.append(combined_loss)
            pos_id_pad.append(cur_pos_id)
        labels_part2_pad = torch.stack(labels_part2_pad)
        loss_labels_pad = torch.stack(loss_labels_pad)
        pos_id_pad = torch.stack(pos_id_pad)
 
        return None, inputs_embeds2_pad, attention_mask2_pad, labels_part2_pad, loss_labels_pad, pos_id_pad
    def prepare_multimodal_data_cache(self, input_ids, attention_mask, image, query, src, tag):
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if cur_input_ids.numel() > 0:
                ...
                cur_new_input_embeds.append(non_img_tokens)
                # else:
                #     assert not (cur_input_ids == self.config.evidence_token_id).any().item()
                #     cur_new_input_embeds.append(non_img_tokens)

            cur_new_input_embeds = [x.to(device=self.device) for x in cur_new_input_embeds]
            cur_new_input_embeds = torch.cat(cur_new_input_embeds)
            # if self.inds is not None:
            #     cur_new_input_embeds2 = [x.to(device=self.device) for x in cur_new_input_embeds2]
            #     cur_new_input_embeds2 = torch.cat(cur_new_input_embeds2)
        ...
        if attention_mask is not None:
            if attention_mask.dim() == 4:   
                ...
                assert attention_mask.shape == new_input_embeds.shape[:2]
                # else:
                #     attention_mask = attention_mask.new_full((attention_mask.shape[0], new_input_embeds.shape[1]), True)
                #     attention_mask2 = attention_mask.new_full((attention_mask.shape[0], new_input_embeds.shape[1]+new_input_embeds2.shape[1]), True)
                #     assert attention_mask.shape == new_input_embeds.shape[:2] and attention_mask2.shape[1] == new_input_embeds2.shape[-2] + new_input_embeds.shape[-2]
    ...
    def dpo_loss(
        ...
        """Compute the DPO loss for a batch of policy model log probabilities.
        ...
        if self.loss_type == "sigmoid":
            ...
            chosen_KL = (policy_chosen_logps).mean().clamp(min=0)
            rejected_KL = (policy_rejected_logps).mean().clamp(min=0)
    ...
    def calculate_logps(self, frm_tokens, evi_tokens, logits, event, relative_offset, average_log_prob: bool = False):
        final_logps = 0
        if event.get('grounding'):
            # print(1)
            distances = []
            frm_index = torch.arange(frm_tokens.size(0), device=frm_tokens.device) 
            for interval in event['grounding']['tgt']:
                start, end = interval
                interval_distances = []
            ...
            input_index = torch.tensor(event['grounding']['input_index'], device=logits.device)
            input_index = input_index + relative_offset
            evi_tokens = evi_tokens[input_index]
            sim = torch.matmul(evi_tokens, frm_tokens.t()) / evi_tokens.size(1)**0.5
            p_sim = torch.sigmoid(sim)
            p_sim = sim_tgt * p_sim + (1 - sim_tgt) * (1 - p_sim)
            result = (p_sim+1e-10).log().sum(dim=(0,1))
            final_logps += result

        if event.get('text'):
            input_index = torch.tensor(event['text']['input_index'], device=logits.device)
            input_index = input_index + relative_offset 
            logit_token_id = torch.tensor(event['text']['logit_token_id'], device=logits.device)
            per_token_logps = torch.gather(logits[input_index].log_softmax(-1), dim=1, index=logit_token_id.unsqueeze(1)).squeeze(1) 
            per_token_logps_check = torch.gather(logits[input_index].unsqueeze(0).log_softmax(-1), dim=2, index=logit_token_id.unsqueeze(0).unsqueeze(2)).squeeze(2)
            assert (per_token_logps == per_token_logps_check[0]).all()
            

            if average_log_prob:
                result = (per_token_logps).sum(-1) / per_token_logps.shape[0]
            else:
                result = (per_token_logps).sum(-1)    
            final_logps += result

        return final_logps
    
    def calculate_focused_dpo_loss(self, frm_tokens, evi_tokens, logits, conflict_events, relative_offset):


        # if len(conflict_events) == 1:
        #     chosen_logps_dpo = self.calculate_logps(frm_tokens, evi_tokens, logits, conflict_events[-1]['chosen'], relative_offset)
        #     reject_logps_dpo = self.calculate_logps(frm_tokens, evi_tokens, logits, conflict_events[-1]['reject'], relative_offset)
        #     dpo_loss_union = self.dpo_loss(chosen_logps_dpo, reject_logps_dpo)

        #     return dpo_loss_union
    ...
    def forward_evi(self,
        ...
        self.model.relative_offset = kwargs['relative_offset']
        self.model.special_mask_list = kwargs['special_mask_list']
        self.pos_id = kwargs['pos_id']

        self.model.stage_2 = False
        ...
        self.model.stage_2 = True
        input_ids_part2, inputs_embeds2, attention_mask2, labels_part2, loss_labels, pos_id = self.prepare_multimodal_data2(
            input_ids_part2, inputs_embeds1, processed_image, img_cnt, attention_mask2, labels_part2, attention_mask1, labels_part1, original_lengths1, loss_labels, src, tgt, tgt_re, img_embed_ori)
        ...
        for s, t in zip(src, tgt):
            _s_gt = []
            # if s is not None:
            #     _s_gt.append([round(x) for x in s])
            #     _s_gt.append([round(x) for x in s])
            if t is not None:
        ...
        for i, (seps, ts) in enumerate(zip(self.model.img_token_pos, times_gt)):     
            ...
            focused_dpo_loss = focused_dpo_loss + self.calculate_focused_dpo_loss(frm_tokens, evi_tokens_all[i], outputs.logits[i], kwargs['conflict_events'][i], kwargs['relative_offset'][i])
            ...
            if self.config.consistency_loss:
                ...
                # cur_src_count = int(len(src[i])/2) if src[i] is not None else 0
        ...
        if avg_factor > 0:
            outputs.loss = outputs.loss + focused_dpo_loss/len(times_gt) + loss_match / avg_factor + l1_loss / avg_factor
        else:
            outputs.loss = outputs.loss + focused_dpo_loss/len(times_gt) + (frm_tokens_all + evi_tokens_all).mean() * 0
