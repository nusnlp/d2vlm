...
for IDX in $(seq 0 $((CHUNKS-1))); do
    port=$((29502 + $IDX))
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} MASTER_PORT=$port python etchat/eval/infer_videomme.py \
        --anno_path $anno_path \
        --data_path /storage/wenzheng/dataset/D2VLM_other_benchmark/Video-MME/data \
        --pred_path $pred_path \
