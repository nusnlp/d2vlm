# ---------------------------------------------
# Modified Evaluation code of E.T. Bench for D2VLM output
# Copyright (c) 2024 Wenzheng Zeng
...
def tvg_format(ans, delimiter):
    ...
    for time_pattern in patterns:
        ...
        if time_matches:
            timestamps = [[float(start.rstrip('.')), float(end.rstrip('.'))] for start, end in time_matches]
        if time_matches_evi:
...
def rvq_eval(samples, st, delimiter):
    ...
    acc = hit / len(samples)
    out = dict(Total=len(samples), Failed=cnt, Acc=round(acc, 5), Matched_count=weakly_match, matched_ratio=weakly_match/len(samples), correct_ratio_within_matched = matched_correct/(weakly_match+1e-7))
    return out
...
def dvc_eval(samples, st, delimiter):
    ...
    for sample in samples:
        ...
        evi[sample['video']] = evi_time

    scale = len(pred['results']) / len(samples)

    if gt_dict:
        evaluator = DVCEval(ground_truth=gt_dict, prediction=pred, evidence = evi, tious=iou_thr, sentsim=st)
        evaluator.evaluate()
        scores = evaluator.scores
    else:
        scores = dict()
        for key in ('Recall', 'Precision', 'Recall_evi', 'Precision_evi', 'Bleu_1', 'Bleu_2', 'Bleu_3', 'Bleu_4', 'METEOR', 'ROUGE_L', 'CIDEr',
                    'SentSim'):
    ...
    out['F1'] = round(sum(f1_score) / len(f1_score), 5)
    f1_score_evi = []
    for rec, prc, thr in zip(scores['Recall_evi'], scores['Precision_evi'], iou_thr):
        rec = rec * scale
        prc = prc * scale
        f1 = 0 if prc + rec == 0 else 2 * prc * rec / (prc + rec)
        out[f'F1_evi@{thr}'] = round(f1, 5)
        f1_score_evi.append(f1)
    out['F1_evi'] = round(sum(f1_score_evi) / len(f1_score_evi), 5)
...
def parse_args():
    ...
    parser.add_argument('--pred_path', default="/storage/wenzheng/model_zoo/0306_final_important/dpo_focused_with_only_dpo_rvc_1loss_1_opensoucre_try/etbench/")
    parser.add_argument('--pred_path', default="/storage/wenzheng/model_zoo/important_checkpoint_collection_HF/202505_mcqa/with_mcqa_dpo_only_letter/etbench/")
...
if __name__ == '__main__':
    ...
    log_file = nncore.join(dir_name, 'metrics2_rvq_looser.log')
    ...
    nncore.log(tabulate(out))
    os.makedirs(os.path.join(dir_name, 'eval_results_debug'), exist_ok=True)
    path = nncore.join(dir_name, f'eval_results_debug/metrics_{args.pred_path.split('/')[-2] + args.pred_path.split('/')[-1]}.json')
