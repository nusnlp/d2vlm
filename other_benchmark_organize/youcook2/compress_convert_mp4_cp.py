...
def process_videos(input_root, output_dir, size=224, fps=6, num_workers=24):
    ...
    all_files = []
    for first_level in Path(input_root).iterdir():
        if first_level.is_dir():
            for file_path in first_level.rglob('*'):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_root", type=str, 
                       default="/storage/wenzheng/dataset/TimeChat/TimeIT/data/dense_video_captioning/youcook2/raw_videos/validation", 
                       help="输入根目录")
    parser.add_argument("--output_dir", type=str, 
                       default="/storage/wenzheng/dataset/D2VLM_other_benchmark/youcook2/compressed_videos/", 
                       help="输出目录")
